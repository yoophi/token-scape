...
가능한 경우 다음 ccusage cache 파일을 우선 사용합니다.

```text
~/.claude/usage-limits.json

Claude Code statusline JSON을 이용한 `~/.claude/token-scope-status.json` 캐시를 지원한다.
Claude Code가 macOS Keychain에 저장한 OAuth 토큰을 읽어 Anthropic OAuth usage API를 우선 조회한다.
OAuth API 응답은 `~/.claude/token-scope-oauth-usage.json`에 캐시하고 5분 이내에는 재호출하지 않는다.
OAuth API 플랜명은 `~/.claude/token-scope-oauth-plan.json`에 별도 캐시하고 24시간 동안 재사용한다.
OAuth API 호출 실패 시 재시도 간격은 5분으로 한다.
OAuth API 429 응답에 Retry-After 헤더가 있으면 그 값을 우선 사용한다.
OAuth API 401/403은 자동 재시도하지 않고 재인증 필요 상태로 표시한다.
사용자가 새로고침 버튼을 수동으로 누를 때는 429 실패 재시도 대기 중이어도 OAuth API를 즉시 직접 호출한다.
OAuth API가 rate limit 등으로 실패하면 최근 30분 OAuth 캐시를 fallback으로 사용한다.
OAuth API 실패 상태와 다음 재시도까지 남은 시간은 하단 status bar 영역에 표시한다.
OAuth API 조회에 실패하면 statusline 캐시를 사용한다.
statusline 캐시는 최근 15분 안에 갱신된 경우 ccusage 캐시보다 우선 사용한다.
사용자가 기존 statusline 스크립트에 연결할 수 있도록 collector 스크립트를 제공한다.
자세히 보기의 Claude 5시간/주간 pie chart는 서버에서 받은 사용률과 reset 시각을 기반으로 남은 사용량 비율과 남은 시간 비율을 표시한다.
```

이 파일이 있으면 Claude Code의 실제 usage limit cache로 간주하고 5시간/7일 사용률, 남은 비율, reset 정보를 표시합니다. 파일이 없거나 파싱할 수 없으면 기존 JSONL 로그 기반 로컬 추정으로 fallback합니다.
...
- Claude의 실제 usage cache가 있으면 `남은 비율`은 cache의 utilization 기반으로 계산합니다.
- Claude의 실제 usage cache가 없으면 활성 usage block의 시간 기준 잔여율을 `블록 잔여`로 표시합니다.
- Claude Code 카드에는 사용량 출처를 표시합니다. 예: `CCUSAGE`, `로컬 추정`.
